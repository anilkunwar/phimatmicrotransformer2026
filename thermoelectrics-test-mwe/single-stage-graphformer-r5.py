...
def render_microtransformer_kg_rag_tab(analysis_data: Dict, ontology: Any):
    ...
    # ─── NLP QUERY PARSING (runs FIRST to set defaults) ─────────────────
    st.markdown("---")
    ...
    QUERY_CONCEPT_MAP = [
        ...
        (r"\bhot\s*press\b.*\bgrain\b", "hot_pressing", "grain_size"),
        (r"\bhot\s*press\b.*\bseebeck\b", "hot_pressing", "seebeck_coefficient"),
        (r"\bball\s*mill\b.*\bgrain\b", "ball_milling", "grain_size"),
        (r"\bnanotwin\b.*\bthermal\b", "nanotwin", "lattice_thermal_conductivity"),
        (r"\balloy\s*scat\b.*\bthermal\b", "alloy_scattering", "lattice_thermal_conductivity"),
    ]

    # Parse query FIRST to determine default dropdown indices
    quick_query = st.text_input(
        "Or type a natural-language question:",
        value="How does doping affect Seebeck coefficient?",
        key="mt_quick_query",
        placeholder="e.g., How does doping affect Seebeck coefficient?",
    )
    ...
    if nlp_src and nlp_tgt:
        ...
        default_tgt_idx = find_option_index("seebeck_coefficient")
    ...
    # Strip tags to get clean concept names
