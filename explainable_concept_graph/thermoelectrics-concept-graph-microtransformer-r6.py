...
def render_microtransformer_kg_rag_tab(analysis_data: Dict, ontology: Any):
    ...
    col1, col2 = st.columns(2)
    with col1:
        selected_mat = st.selectbox("Source Material:", material_nodes, index=0 if material_nodes else None)
    with col2:
        selected_prop = st.selectbox("Target Property:", property_nodes, index=0 if property_nodes else None)
    ...
    st.write("Export the Microtransformer to an INT8 ONNX file (< 2MB) for deployment on edge thin-clients.")
