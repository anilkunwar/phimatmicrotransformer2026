...
def render_microtransformer_kg_rag_tab(analysis_data: Dict, ontology: Any):
    ...
    # --- 2. Build a unified concept list (all types, grouped) ---
    type_order = [
    ...
    for ctype in type_order:
        ...
        if members:
            ...
            grouped_options.extend(members)

    if not grouped_options:
        st.error("No concepts available.")
        return

    separator_set = {s for s in grouped_options if s.startswith("--- ")}

    # --- 3. UI: Two free-form selectors ---
    st.markdown("---")
    st.markdown("#### 🔬 Expert Routing Activation Analysis")
    st.write("Select **any** source and target concept to trace a reasoning path "
             "through the Knowledge Graph and see how the Microtransformer "
             "routes through the 32 latent experts.")
    ...
    with col1:
        src_idx = st.selectbox(
            ...
            index=grouped_options.index("doping_concentration")
            if "doping_concentration" in grouped_options else 0,
            key="mt_src_select",
        )
    with col2:
        tgt_idx = st.selectbox(
            ...
            options=grouped_options,
            index=grouped_options.index("seebeck_coefficient")
            if "seebeck_coefficient" in grouped_options else 1,
            key="mt_tgt_select",
        )

    selected_src = None if src_idx in separator_set else src_idx
    selected_tgt = None if tgt_idx in separator_set else tgt_idx

    if selected_src and selected_tgt and selected_src == selected_tgt:
        st.warning("Source and target must be different concepts.")
        selected_src = None

    # --- 4. Quick-query shortcut ---
    st.markdown("**Or type a natural-language question:**")
    quick_query = st.text_input(
        "e.g., How does doping affect Seebeck coefficient?",
        key="mt_quick_query",
        placeholder="How does doping affect Seebeck coefficient?",
    )

    QUERY_CONCEPT_MAP = [
        (r"\bdoping\b.*\bseebeck\b", "doping_concentration", "seebeck_coefficient"),
        (r"\bseebeck\b.*\bdoping\b", "doping_concentration", "seebeck_coefficient"),
        (r"\bgrain\s*size\b.*\bthermal\s*cond\b", "grain_size", "lattice_thermal_conductivity"),
        (r"\bphonon\s*scat\b.*\bthermal\s*cond\b", "phonon_scattering", "lattice_thermal_conductivity"),
        (r"\bband\s*conv\b.*\bseebeck\b", "band_convergence", "seebeck_coefficient"),
        (r"\btemperature\b.*\bseebeck\b", "temperature", "seebeck_coefficient"),
        (r"\bcarrier\s*scat\b.*\bseebeck\b", "carrier_scattering", "seebeck_coefficient"),
        (r"\bsps\b.*\bgrain\b", "spark_plasma_sintering", "grain_size"),
        (r"\bball\s*mill\b.*\bgrain\b", "ball_milling", "grain_size"),
        (r"\bnanotwin\b.*\bthermal\b", "nanotwin", "lattice_thermal_conductivity"),
    ]

    auto_src, auto_tgt = None, None
        ...
        for pattern, src_concept, tgt_concept in QUERY_CONCEPT_MAP:
            if re.search(pattern, q_lower):
                if src_concept in all_available:
                    auto_src = src_concept
                if tgt_concept in all_available:
                    auto_tgt = tgt_concept
                break
        if auto_src and auto_tgt:
            st.success(
                f"🧠 Parsed query → `{auto_src}` → `{auto_tgt}` "
                f"(click the button below to run inference)"
            )
            selected_src = auto_src
            selected_tgt = auto_tgt
        else:
            st.info("Could not auto-parse query. Select concepts manually above.")
    ...
    if st.button("⚡ Run LatentMoE Inference on Path", type="primary"):
        if not selected_src or not selected_tgt:
        ...
        elif selected_src not in concept_to_id:
            st.warning(
                f"`{selected_src}` is not in the graph. "
                f"It exists in the ontology but wasn't extracted from documents. "
                f"Try lowering the frequency threshold or using query-focused build."
            )
        elif selected_tgt not in concept_to_id:
            st.warning(
                f"`{selected_tgt}` is not in the graph. "
                f"It exists in the ontology but wasn't extracted from documents."
            )
        else:
            path = None
            ...
            # 2. Fallback to ontology inference
            if path is None:
                ...
                if ontology_paths:
                    ...
                    st.info(
                        "Path found in ontology (no direct graph edge). "
                        "Some path nodes may not have graph embeddings."
                    )
                else:
                    st.warning(
                        f"No inference path found between `{selected_src}` "
                        f"and `{selected_tgt}` in the ontology."
                    )
            ...
            for n in path:
                if n in concept_to_id:
                    ...
                    st.caption(
                        f"⚠️ `{n}` not in graph embedding table — using fallback ID 0. "
                        f"Expert routing will still work but node embedding is generic."
                    )
            ...
            fig_heat = px.imshow(
                per_token_df.T,
                labels=dict(x="Path Token", y="Expert Domain", color="Weight"),
                color_continuous_scale="Viridis",
                aspect="auto",
                height=400,
            )
            ...
            top3 = df_experts.head(3)["Expert Domain"].tolist()
            interpretation_map = {
                "Doping Effects": "The model recognizes this path involves dopant-induced changes to electronic structure.",
                "Carrier Concentration": "Routing highlights carrier density as the key mediating variable.",
                "Seebeck Coefficient": "The path terminates at Seebeck — the model allocates capacity to decode S.",
                "Band Structure": "Doping shifts the Fermi level and band edges — the model picks this up.",
                "Band Convergence": "If multiple bands converge, S is enhanced — the expert activates.",
            ...
            try:
                with tempfile.TemporaryDirectory() as tmp_dir:
                    torch.onnx.export(
                        ...
                        f"{tmp_dir}/kg_microtransformer.onnx",
                        input_names=["node_seq", "edge_seq"],
                        output_names=["output", "routing_weights"],
                        dynamic_axes={
                            "node_seq": {0: "batch", 1: "seq_len"},
                            "edge_seq": {0: "batch", 1: "seq_len"},
                            "output": {0: "batch", 1: "seq_len"},
                            "routing_weights": {0: "batch", 1: "seq_len"},
                        },
                        opset_version=14,
                    )
                    with open(f"{tmp_dir}/kg_microtransformer.onnx", "rb") as f:
                        onnx_bytes = f.read()
