...
# ============================================================================
# RUN METRICS LOGGER (LLM + Microtransformer observability)
# ============================================================================
@dataclass
class _StepMetrics:
    name: str
...
class RunMetricsLogger:
    """Lightweight step logger for LLM and MT operations."""
    def __init__(self) -> None:
        self.steps: List[Dict[str, Any]] = []
        self._current_step: Optional[_StepMetrics] = None

    def start_step(self, name: str) -> None:
        self._current_step = _StepMetrics(
            name=name,
            start_time=time.perf_counter(),
            start_mem=get_memory_usage_mb(),
        )

    def end_step(self, metadata: Dict = None) -> None:
        ...
        self.steps.append({
            ...
            "duration_ms": round((end_time - self._current_step.start_time) * 1000, 2),
            "mem_delta_mb": round(end_mem - self._current_step.start_mem, 2),
...
_metrics_logger_instance: Optional[RunMetricsLogger] = None


def get_metrics_logger() -> RunMetricsLogger:
    global _metrics_logger_instance
    if _metrics_logger_instance is None:
        _metrics_logger_instance = RunMetricsLogger()
    return _metrics_logger_instance


def render_live_metrics_panel() -> None:
    logger = get_metrics_logger()
    if not logger.steps:
        return
    st.sidebar.markdown("---")
    st.sidebar.markdown("### 📊 Live Metrics")
    for step in logger.steps[-6:]:
        meta = step.get("metadata", {})
        meta_str = " | ".join(f"{k}={v}" for k, v in list(meta.items())[:2])
        st.sidebar.caption(
            f"**{step['name']}**: {step['duration_ms']} ms | "
            f"Δ{step['mem_delta_mb']:+.1f} MB"
...
# ============================================================================
# UNIFIED MEMORY CACHE SYSTEM (LLM ↔ Graph Isolation)
# ============================================================================
LLM_CACHE_FILE = Path(SCRIPT_DIR) / "_llm_query_cache.json"
ANALYSIS_CACHE_DIR = Path(SCRIPT_DIR) / "_analysis_cache"
ANALYSIS_PKL = ANALYSIS_CACHE_DIR / "analysis_data.pkl"
ANALYSIS_TORCH = ANALYSIS_CACHE_DIR / "analysis_torch.pt"
ANALYSIS_META = ANALYSIS_CACHE_DIR / "analysis_meta.json"


def save_llm_query_to_cache(analysis, ontology, expander):
    cache_data = {
        "query": analysis.original_query,
        "whitelist": list(set(analysis.explicitly_mentioned + analysis.inferred_concepts)),
        ...
        "expanded_ontology": {
            "concepts": {
                k: {
                    ...
                    "related_processes": list(v.related_processes),
                    "related_properties": list(v.related_properties),
                    "definition": v.definition,
                } for k, v in ontology.concepts.items()
            },
            "relationships": [
                {"source": r.source, "target": r.target, "rel_type": r.rel_type.value,
                 "confidence": r.confidence, "evidence": r.evidence, "inferred": r.inferred}
...
def save_analysis_to_cache(analysis_data: dict):
    ANALYSIS_CACHE_DIR.mkdir(exist_ok=True)
    torch_objs = {}
    pkl_objs = {}
    for k, v in analysis_data.items():
        if k == "mt_state_dict" and isinstance(v, dict):
            torch_objs[k] = v
        elif isinstance(v, torch.nn.Module):
            torch_objs[k] = v.state_dict() if hasattr(v, 'state_dict') else v
        elif isinstance(v, torch.Tensor):
            torch_objs[k] = v
        elif k in ('embed_model', 'resolver', 'extractor', 'graph_builder',
                   'qa_factory', 'qa_expander', 'qa_generator'):
            continue
        else:
            try:
                pickle.dumps(v)
    ...
    if torch_objs:
        torch.save(torch_objs, ANALYSIS_TORCH)
    meta = {
        'timestamp': datetime.now().isoformat(),
        'has_torch': bool(torch_objs),
        'keys': list(analysis_data.keys()),
    }
    with open(ANALYSIS_META, 'w') as f:
        json.dump(meta, f)


def load_analysis_from_cache(lightweight: bool = False):
    if not ANALYSIS_PKL.exists():
        return None
    with open(ANALYSIS_PKL, 'rb') as f:
        data = pickle.load(f)
    if not lightweight and ANALYSIS_TORCH.exists():
        torch_objs = torch.load(ANALYSIS_TORCH, map_location='cpu', weights_only=False)
        data.update(torch_objs)
    return data


def clear_analysis_cache():
    for p in [ANALYSIS_PKL, ANALYSIS_TORCH, ANALYSIS_META]:
        if p.exists():
            p.unlink()
...
# ============================================================================
# MICROTRANSFORMER #2: KG-RAG EXTRACTOR (LatentMoE)
# ============================================================================
...
# 32 Highly Specialized Latent Experts for Thermoelectric Domains
TE_EXPERT_LABELS = [
    "General Composition", "Doping Effects", "Alloying Disorders", "Stoichiometry Limits",
    "Band Structure", "Band Convergence", "Resonant Levels", "Density of States",
    "Carrier Concentration", "Carrier Mobility", "Electrical Conductivity", "Seebeck Coefficient",
    "Thermal Conductivity", "Lattice Thermal Cond.", "Electronic Thermal Cond.", "Bipolar Effects",
    "Phonon Scattering", "Grain Boundary Scatter", "Point Defects", "Nanostructuring",
    "SPS Synthesis", "Hot Pressing", "Melt Spinning", "Ball Milling",
    "Phase Stability", "Oxidation Resistance", "Mechanical Robustness", "Thermal Expansion",
    "Stealth Signature", "High-Temp Stability", "ZT Figure of Merit", "Power Factor"
]
...
# ============================================================================
# ONTOLOGY-PRIOR PRE-TRAINING FOR LATENTMOE (U2)
# ============================================================================
TYPE_TO_EXPERTS = {
    ConceptType.PARAMETER:      ["Doping Effects", "Carrier Concentration", "General Composition"],
    ConceptType.PROPERTY:       ["Seebeck Coefficient", "Power Factor", "ZT Figure of Merit"],
    ConceptType.PHENOMENON:     ["Band Convergence", "Resonant Levels", "Phonon Scattering"],
    ConceptType.MATERIAL:       ["Alloying Disorders", "Phase Stability", "General Composition"],
    ConceptType.PROCESS:        ["SPS Synthesis", "Hot Pressing", "Ball Milling", "Melt Spinning"],
    ConceptType.MICROSTRUCTURE: ["Nanostructuring", "Grain Boundary Scatter", "Point Defects"],
}


def prior_train_mt(num_nodes: int, concept_to_id: Dict[str, int], ontology: DomainOntology,
                   steps: int = 60, lr: float = 5e-3, seed: int = 42) -> LatentMoEKGExtractor:
    """Weak-supervision pre-training using ontology concept types."""
    torch.manual_seed(seed)
    model = LatentMoEKGExtractor(num_nodes, NUM_EDGE_TYPES)
    eidx = {n: i for i, n in enumerate(TE_EXPERT_LABELS)}
    tgt = torch.zeros(num_nodes, model.n_experts)
    for c, nid in concept_to_id.items():
        ctype = ontology.get_concept_type(c)
        for n in TYPE_TO_EXPERTS.get(ctype, []):
            if n in eidx:
                tgt[nid, eidx[n]] = 1.0
    tgt /= tgt.sum(1, keepdim=True).clamp_min(1)
    opt = torch.optim.Adam(model.parameters(), lr=lr)
    model.eval()
    seq = torch.arange(num_nodes).unsqueeze(0)
    edge_seq_empty = torch.zeros(1, 0, dtype=torch.long)
    for _ in range(steps):
        opt.zero_grad()
        _, rw = model(seq, edge_seq_empty)
        F.kl_div((rw.squeeze(0) + 1e-8).log(), tgt, reduction="batchmean").backward()
        opt.step()
    return model


def rank_paths_with_mt(model: LatentMoEKGExtractor, paths: List[List[str]],
                       concept_to_id: Dict[str, int]) -> List[List[str]]:
    """Score candidate reasoning paths by router peakedness (confidence)."""
    scored = []
    for p in paths:
        ids = [concept_to_id.get(n, 0) for n in p]
        node_seq = torch.tensor([ids], dtype=torch.long)
        edge_seq = torch.zeros(1, max(len(p) - 1, 0), dtype=torch.long)
        with torch.no_grad():
            _, rw = model(node_seq, edge_seq)
        focus = rw.squeeze(0).max(dim=1).values.mean().item()
        scored.append((focus, p))
    return [p for _, p in sorted(scored, reverse=True)]

def compute_mt_top1_prior_hit(model: LatentMoEKGExtractor, path: List[str],
                              concept_to_id: Dict[str, int], ontology: DomainOntology) -> float:
    """Fraction of tokens where top-1 expert matches TYPE_TO_EXPERTS ontology prior."""
    if not path:
        return 0.0
    ids = [concept_to_id.get(n, 0) for n in path]
    node_seq = torch.tensor([ids], dtype=torch.long)
    edge_seq = torch.zeros(1, max(len(path) - 1, 0), dtype=torch.long)
    with torch.no_grad():
        _, rw = model(node_seq, edge_seq)
    top_experts = rw.squeeze(0).argmax(dim=-1).tolist()
    hits = 0
    eidx = {n: i for i, n in enumerate(TE_EXPERT_LABELS)}
    for i, nid in enumerate(ids):
        # Find canonical concept name from id
        canonical = None
        for c, idx in concept_to_id.items():
            if idx == nid:
                canonical = c
                break
        if canonical is None:
            continue
        ctype = ontology.get_concept_type(canonical)
        expected_experts = TYPE_TO_EXPERTS.get(ctype, [])
        expected_indices = {eidx[n] for n in expected_experts if n in eidx}
        if top_experts[i] in expected_indices:
            hits += 1
    return hits / len(path) if path else 0.0
...
class GraphRAGAnswerGenerator:
    ...
    def generate_ground_response(self, query: str, analysis: QueryAnalysisResult, subgraph: nx.Graph,
                                 ...
                                 max_docs_per_concept: int = 2, routing_prior: Optional[List[Tuple[str, float]]] = None) -> str:
        ...
        if routing_prior:
            prompt += "LatentMoE expert routing prior (emphasize these mechanisms): " + ", ".join(f"{n}={w:.2f}" for n, w in routing_prior) + nl
...
def render_llm_query_panel(ontology: Any, expander: DynamicOntologyExpander, full_graph: nx.Graph) -> Optional[QueryAnalysisResult]:
    ...
    # Show cache status
    if LLM_CACHE_FILE.exists():
        st.sidebar.success("💾 LLM query cache found. Ready to build graph.")
        if st.sidebar.button("🗑️ Clear LLM Query Cache", key="clear_llm_cache_sidebar"):
            clear_llm_query_cache()
            st.session_state.pop('last_query_whitelist', None)
            st.session_state.pop('last_query_analysis', None)
    ...
    metrics.start_step("LLM query analysis")
    with st.sidebar.spinner("Analyzing query..."):
        analysis = analyzer.analyze_query(query, ontology)
    metrics.end_step({"query": query[:50], "n_concepts": len(analysis.all_relevant_concepts)})

    metrics.start_step("Ontology expansion from LLM")
    with st.sidebar.spinner("Expanding ontology..."):
        mutations = expander.apply_query_analysis(analysis, analyzer)
    metrics.end_step({"concepts_added": len(mutations.get("concepts_added", [])),
                      "bridges": len(mutations.get("bridges_created", []))})

    # --- UNLOAD & SAVE TO DISK CACHE ---
    save_llm_query_to_cache(analysis, ontology, expander)

    # Clear heavy session state to enforce reading from disk
    st.session_state.pop('last_query_analysis', None)
    st.session_state.pop('last_query_whitelist', None)

    st.sidebar.success("✅ Analysis complete & saved to temporary cache!")
    st.sidebar.info("👉 LLM unloaded. Click 'Build Concept Graph with Reasoning' on the main page.")

    # Explicitly unload LLM to free RAM for Graph Build
    if hasattr(analyzer, 'unload_model'):
        analyzer.unload_model()
    if 'qa_factory' in st.session_state:
        for an in st.session_state.qa_factory._local_cache.values():
            if hasattr(an, 'unload_model'):
                an.unload_model()
        st.session_state.qa_factory._local_cache.clear()
...
def render_llm_qa_tab(analysis_data: Dict, ontology: Any):
    ...
    st.markdown("""
    Ask a specific scientific question about thermoelectric materials. The system will dynamically expand the ontology, extract a relevant subgraph, and generate a grounded answer using retrieved literature snippets.
    """)

    # --- MEMORY OFFLOAD CONTROLS ---
    col_offload, col_reload = st.columns(2)
    with col_offload:
        if st.button("💾 Offload Graph to Disk & Free Memory", key="offload_graph_btn"):
            if st.session_state.analysis_data is not None:
                with st.spinner("Saving heavy graph objects to disk..."):
                    save_analysis_to_cache(st.session_state.analysis_data)
                    # Clear heavy objects from session state
                    heavy_keys = ['analysis_data', 'gnn_model', 'final_emb', 'embed_model']
                    for k in heavy_keys:
                    ...
                    if torch.cuda.is_available():
                        torch.cuda.empty_cache()
                st.success("✅ Graph offloaded to disk. Memory freed for LLM.")
                try:
                    st.rerun()
                except AttributeError:
                    st.experimental_rerun()
            else:
                st.warning("No graph data in memory to offload.")

    with col_reload:
        if st.button("📂 Reload Full Graph from Disk", key="reload_graph_btn"):
            with st.spinner("Reloading full graph from disk..."):
                data = load_analysis_from_cache(lightweight=False)
                if data:
                    st.session_state.analysis_data = data
                    st.success("✅ Full graph restored from disk.")
                    try:
                        st.rerun()
                    except AttributeError:
                        st.experimental_rerun()
                else:
                    st.error("No graph cache found on disk.")

    # Determine data source: live session state or lightweight disk cache
    live_data = analysis_data
    if live_data is None:
        st.info("Graph not in memory. Loading lightweight graph from disk cache for Q&A...")
        live_data = load_analysis_from_cache(lightweight=True)
        if live_data is None:
            st.error("No graph data available. Please build the graph first.")
            return
    ...
    with col1:
        query = st.text_input("Enter your research question:", placeholder="e.g., How does shell thickness affect the LSPR peak position?")
    with col2:
    ...
    if st.button("🔍 Analyze & Answer", type="primary"):
        ...
        with st.spinner("📚 Retrieving evidence and generating answer..."):
            # Pull routing prior from last MT run if available
            mt_run = st.session_state.get("mt_last_run")
            routing_prior = None
            if mt_run and "avg" in mt_run:
                import pandas as pd
                routing_prior = list(pd.Series(mt_run["avg"], index=TE_EXPERT_LABELS).sort_values(ascending=False).head(5).items())
            answer = generator.generate_ground_response(
                query=query, analysis=analysis, subgraph=subgraph,
                concept_abstract_map=live_data["concept_abstract_map"],
                all_texts=live_data.get("all_texts", []),
...
def render_microtransformer_kg_rag_tab(analysis_data: Dict, ontology: Any):
    ...
    st.markdown("""
    This microtransformer encodes graph traversals (node-edge-node sequences) to extract phase stability 
    and electromechanical links. It uses **Latent Mixture of Experts (l-MoE_acc)** to route tokens through 
    ...
    **Upgrades:** ontology-prior pre-training, path ranking, LLM↔MT cache linkage, ONNX parity check.
    ...
    mt_cache_key = "mt_cache"
    mt_run_key = "mt_last_run"
    ...
    if mt_cache_key not in st.session_state:
        st.session_state[mt_cache_key] = {}
    ...
    # --- Auto-populate from last query analysis ---
    last_analysis = st.session_state.get("last_query_analysis")
    default_src = default_tgt = None
    if last_analysis:
        explicit = last_analysis.explicitly_mentioned
        materials = [c for c in explicit if ontology.get_concept_type(c) == ConceptType.MATERIAL]
        props = [c for c in explicit if ontology.get_concept_type(c) == ConceptType.PROPERTY]
        default_src = materials[0] if materials else (explicit[0] if explicit else None)
        default_tgt = props[0] if props else (explicit[-1] if explicit and len(explicit) > 1 else None)
    ...
    col1, col2 = st.columns(2)
    with col1:
        src_index = material_nodes.index(default_src) if default_src and default_src in material_nodes else 0
        selected_mat = st.selectbox("Source Material:", material_nodes, index=src_index if material_nodes else None)
    with col2:
        tgt_index = property_nodes.index(default_tgt) if default_tgt and default_tgt in property_nodes else 0
        selected_prop = st.selectbox("Target Property:", property_nodes, index=tgt_index if property_nodes else None)

    # --- Run on selected path ---
    ...
    if last_analysis and last_analysis.highlight_paths:
        if st.button("🧪 Run Microtransformer on Query Paths"):
            metrics.start_step("MT query-path batch inference")
            results = []
            for path in last_analysis.highlight_paths:
                if all(n in concept_to_id for n in path):
                    path_key = tuple(path)
                    mt_cache = st.session_state[mt_cache_key]
                    if path_key in mt_cache:
                        avg_weights = mt_cache[path_key]
                        ...
                        edge_seq = torch.zeros(1, max(len(path) - 1, 0), dtype=torch.long)
                        with torch.no_grad():
                            _, routing_weights = kg_model(node_seq, edge_seq)
                        avg_weights = routing_weights.mean(dim=1).squeeze(0).numpy()
                        mt_cache[path_key] = avg_weights
                    top_exp_idx = int(np.argmax(avg_weights))
                    results.append({
                        "Path": " → ".join(path),
                        "Top Expert": TE_EXPERT_LABELS[top_exp_idx],
                        "Confidence": round(float(avg_weights[top_exp_idx]), 3),
                    })
            if results:
    ...
    if st.button("📦 Export to ONNX"):
        metrics.start_step("ONNX export")
            ...
            try:
                with tempfile.TemporaryDirectory() as tmp_dir:
                    ...
                    try:
                        ...
                        o_rt = sess.run(None, {
                            "node_seq": dummy_nodes.numpy(),
                            "edge_seq": dummy_edges.numpy()
                        })
                        rt_rw = o_rt[1]
                        with torch.no_grad():
                            _, torch_rw = kg_model(dummy_nodes, dummy_edges)
                        max_delta = float(np.abs(rt_rw - torch_rw.numpy()).max())
                        st.metric("ONNX parity max|Δ|", f"{max_delta:.2e}")
                        if max_delta > 1e-3:
                            st.warning("⚠️ ONNX parity error exceeds 1e-3 — review export.")
                    except ImportError:
                        st.caption("onnxruntime not installed — skipping parity check.")
                    except Exception as e_parity:
                        st.caption(f"Parity check skipped: {e_parity}")

                    st.success(f"✅ ONNX Export Successful! File size: {len(onnx_bytes)/1024:.1f} KB")
                    st.download_button(
...
def main() -> None:
    ...
    if cached_llm_data:
        if st.button("🗑️ Clear LLM Query Cache & Use Default Ontology", key="clear_llm_cache_main"):
            clear_llm_query_cache()
            st.session_state.pop('last_query_whitelist', None)
            st.session_state.pop('last_query_dynamic_concepts', None)
            st.session_state.pop('last_query_bridge_concepts', None)
            st.session_state.ontology = DomainOntology()
            st.success("LLM cache cleared and ontology reset to default.")
            try:
                st.rerun()
            except AttributeError:
                st.experimental_rerun()
    ...
    if st.session_state.analysis_data is not None:
        ...
        tab_idx += 1
        with tabs[tab_idx]:
            qa_data = st.session_state.get("analysis_data") or load_analysis_from_cache(lightweight=True)
            if qa_data is not None:
                # Use live session state ontology (may not survive lightweight pickle)
                render_llm_qa_tab(qa_data, st.session_state.ontology)
            else:
